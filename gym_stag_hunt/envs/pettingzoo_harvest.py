...
def env(grid_size=(5, 5), screen_size=(600, 600), obs_type='image', enable_multiagent=False, load_renderer=False,
        max_plants=4, chance_to_mature=.1, chance_to_die=.1, young_reward=1, mature_reward=2, max_time_steps=100,
        obs_shape=(42, 42)):
    """
    ...
    env_init = ZooHarvestEnvironment(grid_size, screen_size, obs_type, enable_multiagent, load_renderer, max_plants,
                                     chance_to_mature, chance_to_die, young_reward, mature_reward, max_time_steps,
                                     obs_shape)
...
class ZooHarvestEnvironment(AECEnv):

    metadata = {'render.modes': ['human'], 'name': "pettingzoo_harvest"}
    ...
    def __init__(self, grid_size=(5, 5), screen_size=(600, 600), obs_type='image', enable_multiagent=False,
                 ...
                 mature_reward=2, max_time_steps=100, obs_shape=(42, 42)):
        """
        ...
        super().__init__()
        self.harvest_env = HarvestEnv(grid_size, screen_size, obs_type, enable_multiagent, load_renderer, max_plants,
                                      chance_to_mature, chance_to_die, young_reward, mature_reward)
        self.possible_agents = ["player_" + str(r) for r in range(2)]
        self.agents = self.possible_agents[:]

        self.shape = obs_shape
        observation_space = Box(low=0, high=255, shape=self.shape + self.harvest_env.observation_space.shape[2:],
                                dtype=np.uint8)
        self.observation_spaces = {agent: observation_space for agent in self.possible_agents}
        self.action_spaces = {agent: self.harvest_env.action_space for agent in self.possible_agents}
        self.has_reset = True
    ...
    def accumulated_step(self, actions):
        ...
        self.t += 1
        obs, rewards, done, info = self.harvest_env.step(actions)
    ...
    def render(self, mode='human'):
        self.harvest_env.render(mode)

    def state(self):
        pass

    def close(self):
        self.harvest_env.close()
